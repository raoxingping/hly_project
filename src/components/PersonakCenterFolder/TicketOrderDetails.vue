          ...
          <div class="MyticketOrderDetailsWrapContentDetalis clearfix">
            <div class="MyticketOrderDetailsWrapContentDetalisLeft">
              <div class="MyticketOrderDetailsWrapContentDetalisLeftTop">
                <h5>{{ticketOrderDetail.tm_or_GoodsListName}}</h5>
                <div class="MyticketOrderDetailsWrapContentDetalisLeftParameter clearfix">
                  <!--<span>已选套餐: 江苏</span>-->
                  <span>下单时间: {{ticketOrderDetail.tm_or_PayTime | getUseTime}}</span>
                  <!--<span>出发城市: {{ticketOrderDetail.ts_to_FromPlace}}</span>-->
                  <span>成人: ￥ {{ticketOrderDetail.tm_or_SumPrice}}x{{ticketOrderDetail.tm_or_TicketCount}}</span>
                  <span v-show="ticketOrderDetail.ts_to_ChildCount">儿童: ￥ {{ticketOrderDetail.tm_or_SumPrice}}x{{ticketOrderDetail.tm_or_TicketCount}}</span>
                </div>
                <div class="AllMoney">
                  <strong>订单总金额: ￥{{ticketOrderDetail.tm_or_SumPrice*ticketOrderDetail.tm_or_TicketCount}}</strong>
                </div>
              </div>
              <div class="orderContacts">
                <h5>订单联系人</h5>
                <ul class="orderContactsData">
                  <li>联系人: {{ticketOrderDetail.tm_or_ConnectName}}</li>
                  <li>手机号: {{ticketOrderDetail.tm_or_TelePhone}}</li>
                  <li>邮箱: {{ticketOrderDetail.tm_or_Email}}</li>
                </ul>
              </div>
            </div>
            <div class="MyticketOrderDetailsWrapContentDetalisRight">
              <h2><i></i>订单编号: {{ticketOrderDetail.tm_or_OrderID}}</h2>
              <div class="MyticketOrderDetailsWrapContentDetalisRightContent">
                <p :style="{display:payStatus?'none':'block'}">{{ticketOrderDetail.tm_or_OutStatus | getOutStatus}}</p>
                <div class="pendingPayment">
                  <strong :style="{display:payStatus?'block':'none'}">{{ticketOrderDetail.tm_or_PayState | getTicketStatus}}</strong>
                  <span class="time" :style="{display:payStatus?'none':'block'}">请在20分钟前完成支付 超时订单将关闭</span>
                  <span class="pendingPaymentAllMoney" :style="{display:payStatus?'none':'block'}" >总价:￥{{ticketOrderDetail.tm_or_SumPrice*ticketOrderDetail.tm_or_TicketCount}}</span>
                  <!--<span class="pendingPaymentAllMoneyOut">还需支付：<em>￥12960</em></span>-->
                  <el-button class="button" :style="{display:payStatus?'none':'block'}" @click="goPay(ticketOrderDetail)">去付款</el-button>
                  <span class="cancelOrder" :style="{display:payStatus?'none':'block'}"  @click="cancelOrder(ticketOrderDetail.tm_or_OrderID)">取消订单</span>
                </div>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goPay(item){
        let orderInfo = {};
        orderInfo.oi_OrderName = item.tm_or_GoodsListName;
        orderInfo.OrderID = item.tm_or_OrderID;
        orderInfo.oi_SellMoney = item.tm_or_SumPrice*item.tm_or_TicketCount;
        orderInfo.oi_OrderTypeID = 2
