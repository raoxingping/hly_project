    ...
    <table cellpadding="5">
      ...
      <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
        <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
          ...
          <section class="mask" :style="positions" ref="mask" v-show="isShowMask">
            <p class="price">
              <span>产品全票价格:<i>¥{{newPriceDetail.ts_pp_Price}}</i></span>
              <span>产品半票价格:<i>¥{{newPriceDetail.ts_pp_HalfPrice}}</i></span>
              <span>产品婴童价格:<i>¥{{newPriceDetail.ts_pp_ChildPrice}}</i></span>
            </p>
            <p>
              <span>出发日期:<i>{{newPriceDetail.ts_pp_Date}}</i></span>
            </p>
            <p>
              <span>出发地:<i>{{newPriceDetail.ts_pp_FromPlace}}</i></span>
            </p>
            <p>
              <span>可售人数:<i>{{newPriceDetail.ts_pp_Person}}</i></span>
            </p>
            <p>
              <span>产品_线路编码:<i>{{newPriceDetail.ts_pp_ProduceTourID}}</i></span>
            </p>
            <p>备注:{{newPriceDetail.ts_pp_Remark}}</p>
          </section>
        </td>
      </tr>
      </tbody>
